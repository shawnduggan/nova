...
export class AutoContextService {
	...
	resolveBacklinks(file: TFile): TFile[] {
		...
		const backlinksMap = (this.app.metadataCache as unknown as { getBacklinksForFile?: (f: TFile) => Map<string, unknown> | Record<string, unknown> }).getBacklinksForFile?.(file);
		...
		for (const [path, _linkInfo] of entries) {
